...
class LIT(pl.LightningModule):
    ...
    def __init__(
        ...
        print(f'LIT encoder out dim: {self.encoder.out_dim}')
        
        self.dim = base_dim
        self.head = head

        self.conv_ch = nn.Conv2d(self.encoder.out_dim, self.dim, kernel_size=3, padding=1)
        self.conv_v = nn.Conv2d(self.dim, self.dim, kernel_size=3, padding=1)
        if self.local_attn:
            self.conv_q = nn.Conv2d(self.dim, self.dim, kernel_size=3, padding=1)
            self.conv_k = nn.Conv2d(self.dim, self.dim, kernel_size=3, padding=1)
        ...
        self.conv_freq = nn.Conv2d(
            self.dim, self.pe_encoder.enc_dims // 2, kernel_size=3, padding=1
        )
    ...
    def query_rgb(self, coord, cell=None):
        ...
        for vx in vx_lst:
            for vy in vy_lst:
                ...
                # b, 2 -> b, q, 2
                rel_cell = cell.clone()
                rel_cell = rel_cell.unsqueeze(1).repeat(1, q, 1)
